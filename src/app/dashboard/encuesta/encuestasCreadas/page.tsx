...
export default function Page() {
    ...
    useEffect(() => {
        if (user) {
            console.log(user)
            fetchEncuestasCreadas(user.nombre + " " + user.apellido1).then(res => setEncuestas(res))
        }
    }, [user, refreshKey])
    return (
        <>
            <div>
                <div className="flex justify-between">
                    ...
                    <Button variant={"default"} className="h-10 bg-blue-400 dark:text-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-5">
                    {encuestas.map((v: Encuestas) => (
                        <article key={v._id} className="bg-white dark:bg-slate-950 p-6 rounded-2xl shadow-sm dark:shadow-md border border-gray-200 dark:border-slate-700 hover:shadow-lg dark:hover:shadow-lg dark:hover:shadow-slate-900/30 transform hover:-translate-y-1 transition flex flex-col">
                            <div className="flex items-start gap-3 mb-3 min-w-0">
                                <div className="min-w-0 flex-1">
                                    <h5 className="text-lg font-semibold text-gray-900 dark:text-white leading-7 break-words">{v.title}</h5>
                                ...
                                <button
                                    className="bg-white border-2 border-rose-400 hover:bg-red-500 rounded p-1 transition cursor-pointer dark:bg-red-900 dark:border-red-800 dark:hover:bg-red-500"
                            ...
                            <div className="space-y-3 mt-auto">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 w-full">
                                    <Link href={`/dashboard/encuesta/editarEncuesta/${v.surveyId}`} className="inline-flex w-full sm:w-auto min-w-0 items-center justify-center bg-white text-black border-3 border-blue-400 dark:bg-gray-900 dark:border-gray-700 dark:text-white text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-md hover:bg-blue-400 dark:hover:bg-blue-400 transition text-center leading-tight whitespace-normal break-words">
                                        Editar
                                    ...
                                    <Link href={`/dashboard/encuesta/resultados/${v.surveyId}`} className="inline-flex w-full sm:w-auto min-w-0 items-center justify-center bg-blue-400 dark:bg-blue-800 text-white text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-md hover:bg-blue-700 dark:hover:bg-blue-700 transition text-center leading-tight whitespace-normal break-words">
                                        Ver Respuestas
                                        <svg className="hidden sm:inline w-4 h-4 ms-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 12H5m14 0-4 4m4-4-4-4" /></svg>
                                    </Link>
                                </div>
